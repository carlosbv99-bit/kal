...
def test_status_reflects_module_state(monkeypatch):
    monkeypatch.setattr(vscode_integration, "get_vscode_status", lambda: {"code_cli_available": True, "installed": False})
...
def test_install_returns_message_on_success(monkeypatch):
    monkeypatch.setattr(vscode_integration, "install_extension", lambda: "Extensión de kal instalada en VS Code.")
...
def test_install_maps_integration_error_to_500(monkeypatch):
    ...
    monkeypatch.setattr(vscode_integration, "install_extension", _raise)
    response = client.post("/integrations/vscode/install", headers=_HEADERS)
